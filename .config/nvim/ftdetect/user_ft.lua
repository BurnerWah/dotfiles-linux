...
--     if ft_ignore_pat:match_str(vim.fn.expand("<amatch>")) then
--       vim.cmd("setf " .. ft)
--     end
--   end
-- end
...
agrp.set({
  {
    ["BufNewFile,BufRead"] = {
      ...
      { "*/.cargo/config,*/.cargo/credentials", "setf toml" },
      { "/var/tmp/*.repo", "setf dosini" },
      { "*/dbus-1/*.conf", "setf xml" },
      { "*/share/zsh/history", "setf zshhist" },
      { "*/waybar/config", "setf jsonc" },
      { "*/bat/config", "setf argfile" },
      { "*/silicon/config", "setf argfile" },
      { "lit.*cfg", "setf python" },
      {
