...
function _vmstat_partitions
  command lsblk -lno TYPE,NAME,MAJ:MIN,SIZE,MOUNTPOINT | \
     string match -r '^part\s+.*' | \
     string replace -r '^part\s+(\S+)\s+(\S+)\s+(.*)' '$1\t$2 $3'
end
...
complete -c vmstat -x -s S -l unit       -d "Set display unit" \
  -a "k\t'Kilobytes (1000)' K\t'Kibibytes (1024)'
      m\t'Megabytes (1000000)' M\t'Mebibytes (1048576)'"
